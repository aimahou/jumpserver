...
-  # 开头的行表示注释
-  > 开头的行表示在 mysql 中执行
-  $ 开头的行表示代码，可以在 python3 环境中执行
-  (py3)$ 开头的行表示必须在 python3 环境中执行
...
::
    ...
    # 下载jumpserver与coco
    (py3)$ cd /opt/
    (py3)$ git clone https://github.com/jumpserver/jumpserver.git && cd jumpserver && git checkout master
    (py3)$ git clone https://github.com/jumpserver/coco.git && cd coco && git checkout master

    # 安装依赖 RPM 包
    (py3)$ yum -y install $(cat /opt/jumpserver/requirements/rpm_requirements.txt)
    (py3)$ yum -y install $(cat /opt/coco/requirements/rpm_requirements.txt)

    # 安装 Python 库依赖
    (py3)$ pip install --upgrade pip
    (py3)$ pip install -r /opt/jumpserver/requirements/requirements.txt
    (py3)$ pip install -r /opt/coco/requirements/requirements.txt
...
::
    ...
    # 运行 Jumpserver
    $ cd /opt/jumpserver
    (py3)$ ./jms start all  # 后台运行使用 -d 参数./jms start all -d
    # 新版本更新了运行脚本，使用方式./jms start|stop|status|restart all  后台运行请添加 -d 参数

    # 运行 coco
    $ cd /opt/coco
    $ cp conf_example.py conf.py  # 如果 coco 与 jumpserver 分开部署，请手动修改 conf.py
    (py3)$ ./cocod start  # 后台运行使用 -d 参数./cocod start -d
